...
def find_children(parent_id, files):
    ...
    for file in files:
        ...
        working_id = file.get('parents')[0] if file.get('parents') != None else ""
        print(working_id)
        if working_id == parent_id:
            # build child dict
            child = {'id':file.get('id'),'name':file.get('name'),'parents':file.get('parents'),'children':[]}
...
def get_folders_with_structure(root_id, drive_service):
    files, nextPageToken = get_folders(drive_service)
    while nextPageToken:
        page, nextPageToken = get_folders(drive_service, nextPageToken)
        files.extend(page)
    
    folder_structure = {}
    for file in files:
        if file['id'] == root_id:
            logging.info(f"Root folder has been found.")
